...
# from time import mktime as mktime_0
...
class DateAxisItem(AxisItem):
    ...
    def tickValues(self, minVal, maxVal, size):
        ...
        dt1 = self.date_from_timestamp(minVal)
        dt2 = self.date_from_timestamp(maxVal)
        ...
        if dx > 63072001:  # 3600s*24*(365+366) = 2 years (count leap year)
            ...
            for y in range(dt1.year + 1, dt2.year):
                dt = datetime(year=y, month=1, day=1, tzinfo=timezone.utc)
                majticks.append(mktime(dt))
