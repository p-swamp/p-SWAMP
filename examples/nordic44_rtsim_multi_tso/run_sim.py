...
def main_pmu(qm_kwargs, pmu_publisher_type=PMUPublisher, pmu_kwargs=None):
    ...
    if pmu_kwargs is None:
        pmu_kwargs = {"ip": local_ip, "port": 50000, "publish_frequency": 10}

    pmus = [pmu_publisher_type(**kw) for kw in pmu_kwargs]

    [InterfaceListener.send_interface_init(manager, interface) for interface in [*pmus]]  # [sync_plot, pmus, tw_plot]]
    # sync_plot.start()
    [pmus_.start() for pmus_ in pmus]
    # tw_plot.start()
...
def run_rtsim(**pmu_kwargs):
    ip = 'localhost'
    qm_kwargs = dict(address=(ip, 50001), authkey=b'abracadabra')
...
if __name__ == '__main__':
    ...
    config_no = load_config('config.toml')
    config_se = load_config('config_se.toml')
    
    run_rtsim(
        pmu_kwargs=[
            {
                "ip": config_no["pmus"]["ip"],
                "port": config_no["pmus"]["port"],
                "stations": config_no["stations"],
            },
            {
                "ip": config_se["pmus"]["ip"],
                "port": config_se["pmus"]["port"] + 1,
                "stations": config_se["stations"],
            },
        ]
    )
